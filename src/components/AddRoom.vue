<template>
    <div id="areaHouse">
            ...
            <div class="info-wrap">
                <div class="info">
                    ...
                    <div class="input-wrap input2">
                        <input type="number" v-model="nameInfo.no" placeholder="几室" /><i>室</i>
                        <input type="number" v-model="nameInfo.floor" placeholder="请输入房间号" /><i>房间</i>
                    </div>
                </div>
                <div class="info">
                    <p>房间朝向</p>
                    <div class="input-wrap input4">
                        <div class="item" v-for="(item,index) in list" :key="item.value">
                            <div @click="toClick($event,item,index)" :class="[currentRentIndex == index ? 'active' : '']">
                ...
                <div class="info">
                    ...
                    <div class="input-wrap input4">
                        <div class="item">
                            <div @click="issueClick($event,0)" :class="[currentIssueIndex == 0 ? 'active' : '']">
                                <i></i><span>是</span>
                            </div>
                        </div>
                        <div class="item">
                            <div @click="issueClick($event,1)" :class="[currentIssueIndex == 1 ? 'active' : '']">
                                <i></i><span>否</span>
                            </div>
                        </div>
                    </div>
        ...
        <div class="part part-margin">
            <h3>房屋配置：</h3>
            <div class="info-wrap">
                <div class="item" v-for="(item,key) in faclityObj" :key="key">
                    <div @click="faclityClick($event,key)" :class="[resourceFaclity[key] == 0 ? '' : 'active']">    <i></i><span>{{item}}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    methods: {
        ...
        valiteInfo(){
            ...
            if(Number(floorInfos.num) > Number(floorInfos.allNum)){
                _this.TAlert({
                    show: true,
                    content: '请填写正确的楼层信息'+floorInfos.num+'-----'+floorInfos.allNum,
                    cancel: false,
                    onSure: function (){}
                })
                return false;
        ...
        faclityClick(e,id){
            ...
            console.log(this.resourceFaclity)
        },
        getFaclityInfo(){
            this.updateLoading(true);
